...
public class Messages {
        ...
        private static void loadAllMessages() {
                ...
                NO_PERMISSION = colorize(
                                config.getString("general.no-permission", "&cNo tienes permisos para hacer esto."));
                PLAYER_NOT_FOUND = colorize(config.getString("general.player-not-found", "&cJugador no encontrado."));
                COMMAND_ONLY_PLAYER = colorize(
                                config.getString("general.command-only-player",
                                                "&cEste comando solo puede ser usado por jugadores."));
                CONFIG_RELOADED = colorize(
                                config.getString("general.config-reloaded",
                                                "&aConfiguración recargada correctamente."));
                INVALID_ARGUMENTS = colorize(
                                config.getString("general.invalid-arguments", "&cArgumentos inválidos. Usa: {usage}"));
                ...
                CITY_ALREADY_EXISTS = colorize(
                                config.getString("city.already-exists", "&cYa existe una ciudad con ese nombre."));
                CITY_JOIN_SUCCESS = colorize(
                                config.getString("city.join-success", "&a¡Te has unido a la ciudad '{city}'!"));
                CITY_LEAVE_SUCCESS = colorize(
                                config.getString("city.leave-success", "&a¡Has abandonado la ciudad '{city}'!"));
                CITY_ALREADY_MEMBER = colorize(
                                config.getString("city.already-member", "&cYa eres miembro de una ciudad."));
                CITY_NOT_MEMBER = colorize(config.getString("city.not-member", "&cNo eres miembro de ninguna ciudad."));
                CITY_EXPANDED = colorize(
                                config.getString("city.expanded",
                                                "&a¡Ciudad expandida {blocks} bloques hacia {direction}!"));
                CITY_EXPANSION_FAILED = colorize(
                                config.getString("city.expansion-failed", "&cNo se pudo expandir la ciudad."));
                CITY_INSUFFICIENT_FUNDS = colorize(
                                config.getString("city.insufficient-funds",
                                                "&cLa ciudad no tiene fondos suficientes."));
                CITY_MAX_CITIZENS_REACHED = colorize(
                                config.getString("city.max-citizens-reached",
                                                "&cLa ciudad ha alcanzado el máximo de ciudadanos."));
                ...
                // Mensajes de asedio
                SIEGE_STARTED = colorize(
                                config.getString("siege.started", "&c¡Asedio iniciado! {attacker} vs {defender}"));
                ...
                SIEGE_PROTECTOR_ATTACKED = colorize(
                                config.getString("siege.protector-attacked", "&c¡Protector atacado!"));
                SIEGE_INSUFFICIENT_DEFENDERS = colorize(config.getString("siege.insufficient-defenders",
                                "&cNo hay suficientes defensores conectados ({percentage}% requerido)."));
                SIEGE_ALREADY_ACTIVE = colorize(config.getString("siege.already-active", "&cYa hay un asedio activo."));
                SIEGE_CANNOT_ATTACK_SELF = colorize(
                                config.getString("siege.cannot-attack-self", "&cNo puedes atacar tu propia ciudad."));
                SIEGE_CITY_ATTACKING = colorize(
                                config.getString("siege.city-attacking",
                                                "&cTu ciudad está atacando y no puede ser atacada."));
                SIEGE_CITY_UNDER_ATTACK = colorize(
                                config.getString("siege.city-under-attack",
                                                "&cTu ciudad está bajo ataque y no puede atacar."));
                SIEGE_COOLDOWN_ACTIVE = colorize(config.getString("siege.cooldown-active",
                                "&cHay un cooldown activo entre estas ciudades. Tiempo restante: {time}"));
                SIEGE_FLAG_PLACED = colorize(
                                config.getString("siege.flag-placed", "&c¡Bandera de asedio colocada en {city}!"));
                SIEGE_FLAG_DESTROYED = colorize(
                                config.getString("siege.flag-destroyed", "&a¡Bandera de asedio destruida!"));
                SIEGE_INSUFFICIENT_FUNDS = colorize(config.getString("siege.insufficient-funds",
                                "&cNo tienes suficientes {economy} para iniciar un asedio."));
                SIEGE_INVALID_LOCATION = colorize(
                                config.getString("siege.invalid-location", "&cNo puedes colocar la bandera aquí."));

                // Mensajes de saqueo
                LOOT_PHASE_STARTED = colorize(
                                config.getString("loot.phase-started",
                                                "&c¡Fase de saqueo iniciada! Duración: {duration} minutos"));
                LOOT_PHASE_ENDED = colorize(config.getString("loot.phase-ended", "&a¡Fase de saqueo terminada!"));
                LOOT_FUNDS_STOLEN = colorize(
                                config.getString("loot.funds-stolen",
                                                "&c¡{amount} {economy} robados del banco de la ciudad!"));
                LOOT_CHEST_OPENED = colorize(config.getString("loot.chest-opened", "&6Cofre saqueado por {player}"));
                LOOT_BLOCK_BROKEN = colorize(
                                config.getString("loot.block-broken",
                                                "&6Bloque destruido por {player} durante el saqueo"));
                LOOT_PROTECTION_DISABLED = colorize(
                                config.getString("loot.protection-disabled", "&c¡Protecciones deshabilitadas!"));
                LOOT_PROTECTION_RESTORED = colorize(
                                config.getString("loot.protection-restored", "&a¡Protecciones restauradas!"));

                // Mensajes de impuestos
                TAX_COLLECTED = colorize(
                                config.getString("tax.collected", "&aSe han cobrado {amount} {economy} en impuestos."));
                TAX_INSUFFICIENT_BALANCE = colorize(config.getString("tax.insufficient-balance",
                                "&cNo tienes suficiente dinero para pagar los impuestos."));
                TAX_COLLECTION_NOTICE = colorize(
                                config.getString("tax.collection-notice",
                                                "&6¡Recordatorio! Los impuestos se cobrarán en {time}."));
                TAX_COLLECTION_FAILED = colorize(
                                config.getString("tax.collection-failed", "&cError al cobrar impuestos a {player}."));
                TAX_RATE_CHANGED = colorize(
                                config.getString("tax.rate-changed", "&aTasa de impuestos cambiada a {rate}%."));

                // Mensajes de protección
                PROTECTION_BLOCK_BREAK = colorize(
                                config.getString("protection.block-break",
                                                "&cNo puedes romper bloques en esta ciudad."));
                PROTECTION_BLOCK_PLACE = colorize(
                                config.getString("protection.block-place",
                                                "&cNo puedes colocar bloques en esta ciudad."));
                PROTECTION_INTERACT = colorize(
                                config.getString("protection.interact", "&cNo puedes interactuar en esta ciudad."));
                PROTECTION_PVP = colorize(
                                config.getString("protection.pvp", "&cEl PvP está deshabilitado en esta ciudad."));
                PROTECTION_ENDERPEARL = colorize(
                                config.getString("protection.enderpearl",
                                                "&cNo puedes usar enderpearls en esta ciudad."));
                PROTECTION_MOB_SPAWN = colorize(
                                config.getString("protection.mob-spawn",
                                                "&cLos mobs no pueden aparecer en esta ciudad."));

                // Mensajes de economía
                ECONOMY_DEPOSIT_SUCCESS = colorize(config.getString("economy.deposit-success",
                                "&a{amount} {economy} depositados en el banco de la ciudad."));
                ECONOMY_WITHDRAW_SUCCESS = colorize(
                                config.getString("economy.withdraw-success",
                                                "&a{amount} {economy} retirados del banco de la ciudad."));
                ECONOMY_INSUFFICIENT_FUNDS = colorize(
                                config.getString("economy.insufficient-funds", "&cFondos insuficientes."));
                ECONOMY_BALANCE_SHOW = colorize(
                                config.getString("economy.balance-show", "&6Balance: {amount} {economy}"));
                ECONOMY_TRANSACTION_FAILED = colorize(
                                config.getString("economy.transaction-failed", "&cTransacción fallida."));

                // Mensajes de comandos
                COMMAND_HELP_HEADER = colorize(
                                config.getString("commands.help-header", "&6=== Comandos de CityWars ==="));
                ...
                // Mensajes de eventos
                EVENT_CITY_CREATED_BROADCAST = colorize(
                                config.getString("events.city-created-broadcast",
                                                "&a¡Nueva ciudad '{city}' creada por {player}!"));
                EVENT_CITY_DELETED_BROADCAST = colorize(
                                config.getString("events.city-deleted-broadcast", "&c¡Ciudad '{city}' eliminada!"));
                EVENT_SIEGE_STARTED_BROADCAST = colorize(
                                config.getString("events.siege-started-broadcast",
                                                "&c¡Guerra! {attacker} está atacando {defender}!"));
                EVENT_SIEGE_ENDED_BROADCAST = colorize(
                                config.getString("events.siege-ended-broadcast",
                                                "&a¡Guerra terminada! {winner} vs {loser}"));

                // Títulos y subtítulos
                TITLE_UNDER_ATTACK = colorize(config.getString("titles.under-attack.title", "&c&l¡BAJO ATAQUE!"));
                SUBTITLE_UNDER_ATTACK = colorize(
                                config.getString("titles.under-attack.subtitle", "&fTu ciudad está siendo asediada"));
                TITLE_PROTECTOR_ATTACKED = colorize(
                                config.getString("titles.protector-attacked.title", "&4&l¡PROTECTOR ATACADO!"));
                SUBTITLE_PROTECTOR_ATTACKED = colorize(
                                config.getString("titles.protector-attacked.subtitle", "&fDefiendan al protector"));
                TITLE_SIEGE_VICTORY = colorize(config.getString("titles.siege-victory.title", "&a&l¡VICTORIA!"));
                SUBTITLE_SIEGE_VICTORY = colorize(
                                config.getString("titles.siege-victory.subtitle", "&fHan conquistado {city}"));
                TITLE_SIEGE_DEFEAT = colorize(config.getString("titles.siege-defeat.title", "&c&l¡DERROTA!"));
                SUBTITLE_SIEGE_DEFEAT = colorize(
                                config.getString("titles.siege-defeat.subtitle", "&fSu ciudad ha sido conquistada"));
        }
         ...
         * 
         * @param message      Mensaje original
         * @param placeholders Array de placeholders (placeholder, valor, placeholder,
         *                     valor...)
        ...
        public static String getTaxCollected(BigDecimal amount) {
                return PREFIX + "Se han cobrado " + amount + " de impuestos de tu cuenta.";
        }

        public String getTaxCollectedMessage(BigDecimal amount) {
                return TAX_COLLECTED.replace("{amount}", amount.toString());
        }
}
